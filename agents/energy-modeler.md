...
- **idfkit tooling**: All 32 MCP tools for schema exploration, model editing, validation, simulation, and weather data

## Working Principles

1. **Always validate after changes** — Run `validate_model` after any model modifications
2. **Use batch operations** — Prefer `batch_add_objects` over repeated `add_object` calls
3. **Check references first** — Use `get_available_references` before setting reference fields
4. **Consult the schema** — Call `describe_object_type` before creating unfamiliar object types
5. **Document decisions** — Explain why you chose specific system types, sizes, and parameters
6. **Follow standards** — Reference ASHRAE standards when making design decisions
7. **Think about interactions** — HVAC, envelope, and internal loads are interconnected
